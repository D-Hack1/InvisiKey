...
class _SendMoneyScreenState extends State<SendMoneyScreen> {
  ...
  void _showErrorOverlay({required String message, bool isLockout = false}) {
    if (!mounted || _isDisposed) return;
      ...
      builder: (dialogContext) => Dialog(
        ...
        child: Padding(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Icon(isLockout ? Icons.lock : Icons.cancel, color: Colors.redAccent, size: 48),
              const SizedBox(height: 20),
              Text(
                message,
                textAlign: TextAlign.center,
  ...
  Future<bool> _verifyTransactionPin(String pin) async {
    ...
    if (_txnLocked) {
      ...
      Future.delayed(Duration.zero, () {
        if (!mounted || _isDisposed) return;
        _showErrorOverlay(message: data['message'] ?? 'Transaction locked.', isLockout: true);
      });
      _resetRhythm(); // Reset rhythm after lockout
      return false;
    }
    if (!data['success']) {
      Navigator.of(context, rootNavigator: true).pop(); // Dismiss processing
      String msg = data['message'] ?? 'Incorrect PIN.';
      if (!msg.contains('attempt')) {
        msg += ' ${_txnAttemptsLeft} attempts left.';
      }
      Future.delayed(Duration.zero, () {
        if (_txnLocked || !mounted || _isDisposed) return;
        _showErrorOverlay(message: msg, isLockout: false);
      });
      _resetRhythm(); // Reset rhythm after failed PIN
      return false;
    }
    return true;
  }
  ...
  Future<bool> _verifyTransactionRhythm(List<int> intervals, String button) async {
    ...
    if (!data['success']) {
      Navigator.of(context, rootNavigator: true).pop(); // Dismiss processing
      String msg = data['message'] ?? 'Wrong rhythm.';
      if (!msg.contains('attempt')) {
        msg += ' ${_txnAttemptsLeft} attempts left.';
      }
      Future.delayed(Duration.zero, () {
      ...
      _resetRhythm(); // Reset rhythm after failed rhythm
      return false;
    }
    return true;
  }
    ...
    if (token != null) {
      ...
      if (userMeResponse.statusCode == 200) {
        ...
        if (userMeData["secret_button"] != null) {
          secretButton = userMeData["secret_button"];
          await prefs.setString("secret_button", secretButton!);
        }
      }
    }
  ...
  Widget _buildStepContent() {
    ...
    switch (currentStep) {
      ...
      case 1:
        if (_txnLocked) {
          // Show persistent lockout overlay
          Future.delayed(Duration.zero, () {
            _showErrorOverlay(message: 'Your account is locked for transactions. Please try again later or contact support.');
          });
